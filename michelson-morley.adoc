...
// An antenna transmitting spherical waves at a power \(W_L\), acting
// upon a reflective surface of area \(A_S\) at a distance \(r\), exerts
// a force \(F = \frac{A_S}{2\pi r^2 c}W_L\) on the surface. If the
// surface have mass \(m_S\) and be in free space, it will accelerate at
// rate \(\frac{dv_S}{dt} = \frac{F}{m_S}\).
// 
// Regard this interaction as an elastic collision, transferring a
// differential of momentum. The differential of momentum of the surface
// is \(m_S\,dv_S\). For the waves, on the other hand, the differential
// of momentum is \(c\,dm_L\), where \(dm_L\) is a hypothetical
