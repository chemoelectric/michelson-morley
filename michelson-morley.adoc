...
The radius of one of the foregoing “normalized units” spheres, after
radial scaling by \( 1/{\sqrt{1-( v/c )^2}} \) to account for
wavefront speed, is the same quantity as a longitudinal Doppler shift.
The Doppler shifts are thus the following square roots:
...
// areal scale of \( \frac{1}{ ( 1+v/c )( 1-v/c ) } \). This scale is the
...
// correct and so there _is_ a proof.]footnote:[Okay, I have some ideas
// leading towards proof of this Doppler shift. Probably thanks to
// Imitrex and sleep. The first thing is that we have a wavefront moving
// at relative speed \(1-v/c\) over a relative distance \(1+v/c\), as
// opposed to a wavefront from a stationary antenna, where both those
// quantities are unity. Also the radius of the wavefront is \(1+v/c\)
// instead of unity. Thus we have every factor in the expression that
// then gets square-rooted!]
// 
// [.text-center]
