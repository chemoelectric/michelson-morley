...
Consider the history of a circular wavefront, so that this history
forms a cylinder. Now let us cut a cross section that spans a time
interval. The result is an ellipse. This is a sort of ellipse we might
want for studying Doppler shift. This is one example of why it could
be useful to have a reference on the geometry of ellipses.
...
where the sinusoidal function can vary.
